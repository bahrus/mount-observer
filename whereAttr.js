 ...
 */
function checkBranch(element, prefix, baseName, baseDelimiter, branchKey, subBranches) {
    ...
    const subBranchMatches = subBranches.some(subBranch => {
        ...
        if (typeof subBranch === 'object') {
            ...
            return Object.entries(subBranch).some(([key, nestedBranches]) => {
                return checkNestedBranch(element, branchAttrName, key, nestedBranches);
            });
        }
        return false;
    });
 ...
 */
function checkNestedBranch(element, parentAttrName, branchKey, subBranches) {
    ...
    return subBranches.some(subBranch => {
        ...
        if (typeof subBranch === 'object') {
            ...
            return Object.entries(subBranch).some(([key, nestedBranches]) => {
                return checkNestedBranch(element, attrName, key, nestedBranches);
            });
        }
        return false;
    });
