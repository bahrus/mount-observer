...
This requirement is to support a built in handler similar to the MountObserverScript handler.  The MountObserScript handler looks for type=mountobserver script elements.

This one instead looks for emc script elements:
...
If there are opportunities to share code between mountobserver and this EMCScript handler, feel free to do so via a common module.
...
Once the JSON is parsed, it should be attached to the "export" property of the script element, and event resolved should fire.
...
4.  Using the enhancementConfig found in either step 2 or step 3, spawn an instance of the enhancement class via:
